...
export class ControlDecorator {
  ...
  private updateQsysDesign(property: keyof IControl, value: string | number | boolean): void {
    ...
    // Exclude the Value property without creating an unused variable
    const controlWithoutValue = Object.fromEntries(
      Object.entries(this.control).filter(([key]) => key !== 'Value')
    ) as IControl
